...
class Reaction(commands.Cog, name="message_reactions"):
    ...
    async def generic_react(self, message: discord.Message,
        ...
        if db_entry and current_time - db_entry["last_react"] <= timer:
            return False
        ...
        if db_entry:
            self.bot.cursor.execute(f"UPDATE react_timer SET last_react={current_time} WHERE rowid={db_entry['rowid']}")
        else:
